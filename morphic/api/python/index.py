...
# ─────────────────────────────────────────────
# Text Splitter (char-based, used for TXT/DOCX)
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# Token-based chunker (used for PDF — matches Colab pipeline)
# ─────────────────────────────────────────────
_tokenizer = tiktoken.get_encoding("cl100k_base")
PDF_CHUNK_SIZE_TOKENS = 800
PDF_CHUNK_OVERLAP_TOKENS = 120
...
def _chunk_by_tokens(text: str, chunk_size: int = PDF_CHUNK_SIZE_TOKENS, overlap: int = PDF_CHUNK_OVERLAP_TOKENS) -> List[str]:
    ...
    while start < total:
        ...
        chunk = _tokenizer.decode(tokens[start:end]).strip()
        if chunk:
            chunks.append(chunk)
        if end == total:
            break
...
def _safe_vector_prefix(filename: str) -> str:
    name = re.sub(r"[^a-z0-9]+", "-", filename.lower()).strip("-")
    return name[:40] if name else "doc"
...
# ─────────────────────────────────────────────
# PDF UPLOAD — token-based chunking, page-level extraction
# ─────────────────────────────────────────────
@app.post("/upload-pdf")
async def upload_pdf(
    ...
    try:
        ...
        # Extract text page by page (preserves page context for metadata)
        ...
        logger.info(f"{file.filename}: {len(pages)} pages extracted")

        # Build chunk records with rich metadata
        records: List[Dict[str, Any]] = []
        prefix = _safe_vector_prefix(file.filename)
        for page in pages:
            for chunk_idx, chunk in enumerate(_chunk_by_tokens(page["text"])):
                source_id = f"{file.filename}|p{page['page_number']}|c{chunk_idx}"
                digest = hashlib.sha256(source_id.encode()).hexdigest()[:24]
                vector_id = f"{prefix}-{page['page_number']}-{chunk_idx}-{digest}"
                records.append({
                    "id": vector_id,
                    "text": chunk,
                    "metadata": {
                        ...
                        "total_chunks": 0,  # filled below after all chunks are known
                        "namespace": namespace,
                        "timestamp": ts,
                    },
                })
        ...
        for r in records:
            r["metadata"]["total_chunks"] = total_chunks

        logger.info(f"{file.filename}: {total_chunks} chunks created")

        # Embed in batches of 50
        texts = [r["text"] for r in records]
        embeddings: List[List[float]] = []
        for b in range(0, len(texts), 50):
            embeddings.extend(generate_embeddings(texts[b:b + 50]))
        ...
        for b in range(0, len(vectors), 100):
            index.upsert(vectors=vectors[b:b + 100], namespace=namespace)
            logger.info(f"Upserted batch {b // 100 + 1}/{(len(vectors) - 1) // 100 + 1}")
