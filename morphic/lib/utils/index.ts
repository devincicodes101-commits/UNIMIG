...
/**
 * Ensures a URL is absolute for server-side fetch calls.
...
export function getAbsoluteUrl(path: string): string {
  ...
  // In Vercel, VERCEL_URL is only the domain, not the protocol
...
/**
 * Returns headers needed to bypass Vercel Deployment Protection for internal
 * server-to-server fetches. Returns an empty object if no bypass secret is set.
 */
