          ...
          <div className="divide-y divide-border/40">
            {conversations.map(conv => (
              <div key={conv.id} className="px-6 py-4">
                <div
                  className="flex items-start justify-between cursor-pointer group gap-4"
                  onClick={() => setExpanded(expanded === conv.id ? null : conv.id)}
                >
                  <div className="flex-1 min-w-0 pr-2">
                    <div className="flex flex-wrap items-center gap-2 mb-1.5">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium whitespace-nowrap ${roleBadgeColor[conv.role] || 'bg-muted text-muted-foreground'}`}>
                        {conv.role}
                      </span>
                      <span className="text-xs text-muted-foreground break-all">{conv.user_email || conv.user_id}</span>
                      <span className="text-xs text-muted-foreground/60 w-full sm:w-auto sm:ml-auto">{new Date(conv.timestamp).toLocaleString()}</span>
                    </div>
